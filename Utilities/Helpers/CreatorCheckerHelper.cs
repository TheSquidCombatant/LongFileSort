...
public static class CreatorCheckerHelper
{
    ...
    private static void CheckFileSize(CreatorOptions options)
    {
        ...
        if (sourceFileInfo.Length == 0) throw new FileNotFoundException(exceptionIsEmpty);
    ...
    private static void CheckStringsDuplication(CreatorOptions options, LongFileIndex index)
    {
        ...
        (index as ILargeList<IndexBlock>).Sort(0, index.LongCount(), comparer);

        for (long i = 0; i < index.LongCount() - 1; ++i)
        {
            if (IndexBlockComparer.StringPartCoparison(index[i], index[i + 1]) == 0)
            {
